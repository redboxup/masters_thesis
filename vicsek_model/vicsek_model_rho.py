...
def vicsek_model(r,nn,v0,xlim,ylim,eta,dt):
	for i in range(nn):
...
M=20
Na = np.linspace(100,1000,M)
Va = [0]*M
eta = 0.2
for i in range(M):
	N = int(Na[i])
	Va[i] = vicsek_model(r,nn,v0,xlim,ylim,eta,dt)
	print(Va[i])
	percent = ((i+1)/M)*100
...
name = "Va_vs_rho_f""_"+str(nn)+"_"+current_time+".csv"
